...
- [ ] Import OpenAI client from openai library
- [ ] Import Path from pathlib
- [ ] Import base64 module
- [ ] Import io module
- [ ] Define main() function
- [ ] Parse pdf_path from sys.argv[1]
- [ ] Parse output_format from sys.argv[2] with default to 'markdown'
- [ ] Validate PDF file exists using Path.exists()
- [ ] Print error message and exit with code 1 if PDF not found
- [ ] Validate PDF is readable by attempting to open it
- [ ] Print error message and exit with code 1 if PDF is corrupt
- [ ] Initialize OpenAI client with api_key="EMPTY"
- [ ] Configure base_url to "http://localhost:8080/v1"
- [ ] Configure timeout to 3600 seconds
- [ ] Open PDF document using fitz.open()
- [ ] Initialize empty results list
- [ ] Loop through all pages using range(doc.page_count)
- [ ] Convert each page to image using page.get_pixmap(dpi=144)
- [ ] Convert pixmap to PNG bytes
- [ ] Encode image bytes to base64
- [ ] Prepare OCR request with image data URL
- [ ] Set prompt to "Free OCR."
- [ ] Configure max_tokens to 8192
- [ ] Configure temperature to 0.0
- [ ] Configure extra_body with skip_special_tokens=False
- [ ] Configure vllm_xargs with ngram_size=30
- [ ] Configure vllm_xargs with window_size=90
- [ ] Configure vllm_xargs with whitelist_token_ids=[128821, 128822]
- [ ] Send OCR request using client.chat.completions.create()
- [ ] Extract OCR response text
- [ ] Append page number prefix to result
...
- [ ] Make Python script executable with chmod +x

### 1.4 Create pyproject.toml
- [ ] Create `~/opencode-ocr/pdf-ocr/pyproject.toml`
- [ ] Define project metadata (name, version, description)
- [ ] Set requires-python to ">=3.10"
- [ ] Add openai dependency with version constraint
- [ ] Add PyMuPDF dependency with version constraint
- [ ] Add Pillow dependency with version constraint
- [ ] Verify dependencies are correctly specified

### 1.5 Install Python Dependencies with uv
- [ ] Navigate to `~/opencode-ocr/pdf-ocr/`
- [ ] Run `uv sync` to install dependencies
- [ ] Verify uv.lock file is created
- [ ] Verify all packages are installed successfully
- [ ] Test running backend with `uv run pdf_ocr_backend.py --help` or similar
...
- [ ] Create deployment directory: `mkdir -p ~/.config/opencode/tool/`
- [ ] Copy `pdf-ocr.ts` from repository to deployment directory
- [ ] Copy `pdf_ocr_backend.py` from repository to deployment directory
- [ ] Copy `pyproject.toml` from repository to deployment directory
- [ ] Run `uv sync` in deployment directory to install dependencies
- [ ] Update execute path in pdf-ocr.ts if needed for deployment

### 2.1 Prepare Test Environment
- [ ] Navigate to `~/deepseek-ocr-testing/` directory
...
- [ ] Record processing time for single-page PDF
- [ ] Record processing time for multi-page PDF
